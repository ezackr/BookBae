...
To run this application, you will need a Jakarta EE server. We use glassfish. The server will come with instructions that will tell you how to deploy `.war` files like the `api.war` file built by this application. The application can often be deployed by uploading `api.war` to the server's builtin web management interface.
...
Additionally, you need to unset the following properties (so Glassfish uses the system defaults, which are generally much more appropriate):

- javax.net.ssl.keyStore
- javax.net.ssl.trustStore

These changes can be made under the options tab found by following Configurations > server-config > JVM Settings, which takes properties in the format `-Dpropertyname=value`
